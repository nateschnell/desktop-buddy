# Claude Buddy — desktop app & bridge

Claude Buddy is a little hardware companion for Claude Code: an ESP32
...
`claude-buddy` daemon that bridges Claude Code to the device — and publishes the
...
[latest release](https://github.com/nateschnell/desktop-buddy/releases/latest):
...
- `bridge/` — the Rust crate that builds two binaries: `claude-buddy` (the
  background daemon / CLI) and `claude-buddy-app` (the desktop GUI, `--features
  gui`).
